...
class StressTest500: public ::testing::Test {
protected:
...
TEST_F(StressTest500, StressTest500_1) {
    auto armiesList = factory->generateArmies(500);
    auto battle = battleFactory->createBattle(armiesList);
    EXPECT_NO_THROW(battle->start_war_test());
}

TEST_F(StressTest500, StressTest500_2) {
    auto armiesList = factory->generateArmies(500);
    auto battle = battleFactory->createBattle(armiesList);
    EXPECT_NO_THROW(battle->start_war_test());
}

TEST_F(StressTest500, StressTest500_3) {
    auto armiesList = factory->generateArmies(500);
    auto battle = battleFactory->createBattle(armiesList);
    EXPECT_NO_THROW(battle->start_war_test());
}

TEST_F(StressTest500, StressTest500_4) {
    auto armiesList = factory->generateArmies(500);
    auto battle = battleFactory->createBattle(armiesList);
    EXPECT_NO_THROW(battle->start_war_test());
}

TEST_F(StressTest500, StressTest500_5) {
    auto armiesList = factory->generateArmies(500);
    auto battle = battleFactory->createBattle(armiesList);
    EXPECT_NO_THROW(battle->start_war_test());
}
